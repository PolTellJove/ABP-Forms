<?php 
    ...
    function getPendingsPolls(){
        try {
            ...
            $startSession->execute();
            writeInLog("SQL", "SELECT u.email as email, u.username as user, p.title as poll, p.ID as pollID, u.ID as userID FROM abp_poll.user u INNER JOIN abp_poll.student_poll sp ON u.ID = sp.studentID INNER JOIN abp_poll.poll p ON sp.pollID = p.ID WHERE sp.reply = 0 and sp.send = 0 LIMIT 5;", 'CRON');
            foreach($startSession as $user){
                sendingPoll($user['userID'], $user['pollID']);
                sendEmail($user['email'], 'Enquesta pendent', $user['poll']);
            }
        }catch (PDOException $e) {
            writeInLog("E", "Error:" . $e->getMessage());
        }
